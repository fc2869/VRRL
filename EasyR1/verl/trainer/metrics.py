...
def compute_random_restart_metrics(batch: DataProto) -> dict[str, Any]:
    """Extract random restart training metrics from batch if available.
    
    Only includes random_start samples (excludes prefix_buffer samples,
    which are tracked separately under prefix_buffer/).
    """
    metrics = {}
    
    if "start_turn" not in batch.non_tensor_batch:
        return metrics

    start_turns = batch.non_tensor_batch["start_turn"]
    if len(start_turns) == 0:
        return metrics

    # Filter to random_start samples only
    is_pb = batch.non_tensor_batch.get("is_prefix_buffer_rollout", None)
    if is_pb is not None and len(is_pb) == len(start_turns):
        mask = ~np.array(is_pb, dtype=bool)
        start_turns = np.array(start_turns)[mask]

    if len(start_turns) == 0:
        return metrics

    metrics["random_restart/avg_start_turn"] = float(np.mean(start_turns))
    
    for turn_num in range(1, 9):
        count = np.sum(start_turns == turn_num)
        metrics[f"random_restart/start_turn_{turn_num}_ratio"] = float(count / len(start_turns))
