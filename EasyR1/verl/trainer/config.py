...
class AlgorithmConfig:
    ...
    void_trace_filtering: bool = False
...
class PPOConfig:
    ...
    def post_init(self):
        ...
        self.worker.rollout.train_files = self.data.train_files
        ...
        # Propagate reward_distance_threshold from reward function kwargs to rollout config
        try:
            if getattr(self.worker.rollout, 'reward_distance_threshold', -1) is not None and self.worker.rollout.reward_distance_threshold < 0:
                reward_kwargs = getattr(self.worker.reward, 'reward_function_kwargs', {}) or {}
                dist_thr = int(reward_kwargs.get('distance_threshold', 20))
                self.worker.rollout.reward_distance_threshold = dist_thr
        except Exception:
            # Fallback to default of 20 if anything is missing
            if getattr(self.worker.rollout, 'reward_distance_threshold', -1) is not None and self.worker.rollout.reward_distance_threshold < 0:
                self.worker.rollout.reward_distance_threshold = 20

        # Propagate use_bbox from reward function kwargs to rollout config
        try:
            reward_kwargs = getattr(self.worker.reward, 'reward_function_kwargs', {}) or {}
            use_bbox_val = reward_kwargs.get('use_bbox', False)
            if isinstance(use_bbox_val, str):
                use_bbox_val = use_bbox_val.lower() in ("true", "1", "yes")
            else:
                use_bbox_val = bool(use_bbox_val)
            self.worker.rollout.use_bbox = use_bbox_val
        except Exception:
            pass

        # If potential_sigma is not specified in reward_function_kwargs, use distance_threshold
        # If crop_size is not specified in reward_function_kwargs, derive from worker.rollout.crop_w/crop_h
        try:
            reward_kwargs = getattr(self.worker.reward, 'reward_function_kwargs', {}) or {}
            
            # Default potential_sigma to distance_threshold if not specified
            if 'potential_sigma' not in reward_kwargs:
                dist_thr = float(reward_kwargs.get('distance_threshold', 20))
                reward_kwargs['potential_sigma'] = dist_thr
            
            # Default crop_size to the smaller of crop_w/crop_h (used only for logging)
            if 'crop_size' not in reward_kwargs:
                rollout_crop_w = getattr(self.worker.rollout, 'crop_w', 200)
                rollout_crop_h = getattr(self.worker.rollout, 'crop_h', 200)
                rollout_crop_size = min(rollout_crop_w, rollout_crop_h) if (rollout_crop_w and rollout_crop_h) else 200
                reward_kwargs['crop_size'] = int(rollout_crop_size) if rollout_crop_size else 200
            
            self.worker.reward.reward_function_kwargs = reward_kwargs
        except Exception:
            pass  # Keep existing values if anything fails
