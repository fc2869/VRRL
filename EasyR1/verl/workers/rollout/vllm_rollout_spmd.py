...
from ...models.transformers.qwen2_vl import get_rope_index as get_rope_index_qwen2
from ...models.transformers.qwen3_vl import get_rope_index as get_rope_index_qwen3
...
class vLLMRollout(BaseRollout):
    def __init__(self, model_path: str, config: RolloutConfig, tokenizer: PreTrainedTokenizer, processor: AutoProcessor):
        ...
        if mm_cache_gb is not None:
            add_engine_arg_if_supported(vllm_init_kwargs, "mm_processor_cache_gb", mm_cache_gb)

        # Allow overriding the torch.compile cudagraph mode. This mirrors the
        # CLI flag `--compilation_config.cudagraph_mode=PIECEWISE` documented in
        # the Qwen3-Next vLLM recipe as a workaround for the "illegal memory
        # access" fault in vLLM V1's static CUDA launcher on multi-modal models.
        cudagraph_mode = getattr(config, "cudagraph_mode", None)
        if cudagraph_mode is not None:
            vllm_init_kwargs["compilation_config"] = {"cudagraph_mode": cudagraph_mode}
        ...
        for key in config.to_dict().keys():
            if key == "seed":
                # See multi_turn_rollout_frozenlake.py for the bug rationale:
                # SamplingParams.seed pins per-sequence sampling PRNG ->
                # identical-prompt replicas collapse to identical outputs ->
                # GRPO group variance = 0 -> no gradient. The engine seed is
                # set separately in the LLM() constructor (init randomness).
                continue
...
"""   
    ...
    def generate_sequences(self, prompts: DataProto) -> DataProto:
        ...
        for idx, prompt_len in enumerate(valid_prompt_len):
            ...
            # Use correct get_rope_index based on model type.
            # NOTE: Qwen3-VL reuses Qwen2VLImageProcessorFast as its image_processor, so inspect
            # the TOP-LEVEL processor class ("Qwen3VLProcessor"), not the inner image_processor.
            # See verl#4483.
            processor_class_name = self.processor.__class__.__name__ if self.processor else ""
            get_rope_index = get_rope_index_qwen3 if "Qwen3" in processor_class_name else get_rope_index_qwen2
            ...
            if position_ids.size(1) != new_position_ids.size(0):
                raise ValueError(
                    f"position_ids channel mismatch: got {position_ids.size(1)}, expected {new_position_ids.size(0)}"
                )
            assert (
                torch.sum(position_ids[idx, :, -prompt_len:].cpu() == new_position_ids[:, :prompt_len].cpu())
                == prompt_len * new_position_ids.size(0)
            ), f"Position IDs mismatch at batch index {idx}"
