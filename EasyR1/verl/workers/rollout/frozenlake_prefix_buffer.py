...
"""
Prefix Buffer for on-policy prefix replay in GRPO training (FrozenLake variant).
...
- Different questions
- Different prefix lengths (number of route turns)
...
class PrefixBufferEntry:
    ...
    # FrozenLake map specification (layout, start_pos, level, ...).
    map_spec: dict

    # Ground truth for the question (the GT map spec)
    ground_truth: dict
    ...
    # Number of route turns in this prefix (1, 2, 3, ...)
    num_route_turns: int

    # The final route's action list before terminate was called
    last_actions: List[str]

    # Progress rate (PR) of the final committed actions
    final_pr: float

    # Rendered feedback images (PIL.Image) produced during the prefix turns
    feedback_images: List[Image.Image]

    # The global step when this entry was collected (for staleness tracking)
    collection_step: int

    # Base map image (PIL.Image) for THIS entry's question, captured at collection
    # time. When a buffer entry is replayed into a new rollout (potentially for a
    # different data item), the new rollout must use this base map (not the data
    # item's base map) so the conversation text, base image, and feedback images
    # all refer to the SAME underlying question.
    base_map_image: Optional[Image.Image] = None
...
class PrefixBuffer:
    ...
    def __init__(
        ...
        self._entries: List[PrefixBufferEntry] = []
        ...
        self._entries_by_turns: Dict[int, List[int]] = defaultdict(
            list
        )  # turn_count -> indices
        self._entries_by_question: Dict[str, List[int]] = defaultdict(
            list
        )  # question_id -> indices
    ...
    def _rebuild_indices(self):
        ...
        for idx, entry in enumerate(self._entries):
            ...
            self._entries_by_turns[entry.num_route_turns].append(idx)
    ...
    def add(self, entry: PrefixBufferEntry) -> bool:
        ...
        if len(q_indices) >= self.max_per_question:
            ...
            evict_idx = self._pick_question_evict_idx(q_id, entry.num_route_turns)
    ...
    def _pick_question_evict_idx(
        self, question_id: str, new_turn_count: int
    ) -> Optional[int]:
        """
        ...
        for idx in q_indices:
            if idx < len(self._entries):
                turn_count_to_indices[self._entries[idx].num_route_turns].append(idx)

        if new_turn_count not in turn_count_to_indices:
            # New turn count -> evict oldest from the most over-represented turn count
            most_common_turn = max(
                turn_count_to_indices, key=lambda t: len(turn_count_to_indices[t])
            )
    ...
    def _adjust_sampled_indices_after_pop(self, removed_idx: int):
        ...
        When _entries.pop(removed_idx) is called, every entry that was at a
    ...
    def sample(self, n: int = 1) -> List[PrefixBufferEntry]:
        ...
        2. Within each type, prefer diversity in num_route_turns
        ...
        """
        if len(self._entries) < self.min_size:
            return []
        ...
        if shortage > 0:
            ...
            all_fresh = self._get_fresh_indices(list(range(len(self._entries))))
            all_available = self._get_unsampled_indices(all_fresh)
            remaining_available = [
                idx for idx in all_available if idx not in already_sampled
            ]
    ...
    def _stratified_sample(
        self, available_indices: List[int], n: int
    ) -> Tuple[List[int], List[PrefixBufferEntry]]:
        """
        Sample n entries with stratification by num_route_turns.
        ...
        # Group available indices by num_route_turns
        by_turns: Dict[int, List[int]] = defaultdict(list)
        for idx in available_indices:
            if idx < len(self._entries):
                turns = self._entries[idx].num_route_turns
        ...
        while len(sampled_entries) < n and any(by_turns.values()):
            ...
            if by_turns[turn_count]:
                ...
                if idx < len(self._entries):
                    ...
                    sampled_entries.append(self._entries[idx])
            turn_idx += 1
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "buffer_utilization": (
                len(self._entries) / self.max_size if self.max_size > 0 else 0
            ),
        }
        ...
        stats["buffer_wrong_ratio"] = (
            n_wrong / len(self._entries) if self._entries else 0
        )

        # Count by number of route turns
        turn_counts = defaultdict(int)
        for entry in self._entries:
            turn_counts[entry.num_route_turns] += 1

        total = len(self._entries) if self._entries else 1
        ...
        stats["buffer_fresh_pct"] = len(all_fresh) / total * 100 if self._entries else 0

        # Unsampled entries count
        unsampled = self._get_unsampled_indices(list(range(len(self._entries))))
    ...
    def __repr__(self) -> str:
        return f"PrefixBuffer(size={len(self._entries)}/{self.max_size}, wrong={len(self._wrong_entries)}, right={len(self._right_entries)})"
