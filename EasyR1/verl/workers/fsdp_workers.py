...
class FSDPWorker(Worker):
    ...
    def _build_model_optimizer(
        ...
        # Apply LoRA to actor/critic if enabled
        ...
        # Report trainable parameter count after all freezing/LoRA
    ...
    def _build_rollout(self) -> None:
        ...
        # Check if multi-turn rollout is requested
        if hasattr(self.config.rollout, 'name') and self.config.rollout.name == 'table_multi_turn':
        ...
        elif hasattr(self.config.rollout, 'name') and self.config.rollout.name == 'frozenlake_multi_turn':
            from .rollout import MultiTurnRolloutFrozenLake
            self.rollout = MultiTurnRolloutFrozenLake(
                model_path=self.config.actor.model.model_path,
                config=self.config.rollout,
                tokenizer=self.tokenizer,
                processor=self.processor,
    ...
    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def get_prefix_buffer_stats(self) -> dict:
        """Get prefix buffer statistics for logging."""
        assert self._has_rollout
        if hasattr(self.rollout, 'get_prefix_buffer_stats'):
            return self.rollout.get_prefix_buffer_stats()
        return {}

    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def update_prefix_buffer_step(self, step: int):
        """Update the current step in the prefix buffer for staleness tracking."""
        assert self._has_rollout
        if hasattr(self.rollout, 'update_prefix_buffer_step'):
            self.rollout.update_prefix_buffer_step(step)

    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def reset_prefix_buffer_sampling(self):
        """Reset the sampling state for a new epoch."""
        assert self._has_rollout
        if hasattr(self.rollout, 'reset_prefix_buffer_sampling'):
            self.rollout.reset_prefix_buffer_sampling()
