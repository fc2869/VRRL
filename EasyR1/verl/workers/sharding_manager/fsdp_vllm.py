...
class FSDPVLLMShardingManager(BaseShardingManager):
    ...
    def _sync_weight_to_vllm(self):
        ...
        # Avoid merging LoRA adapters during training sync. Merging would fold LoRA weights into the base
        # parameters and keep them "merged", which disables the low-rank trainable parameters and breaks
        # gradient updates on subsequent optimization steps. Instead, keep adapters active and strip the
        # prefix (e.g. `base_layer.`) so vLLM receives the correct effective weights.
        base_module = getattr(self.module, "_fsdp_wrapped_module", None)
        merged = False
        if base_module is not None and hasattr(base_module, "merge_adapter") and callable(base_module.merge_adapter):
            try:
                base_module.merge_adapter()
                merged = True
            except Exception:
                merged = False
        ...
        if merged and base_module is not None and hasattr(base_module, "unmerge_adapter") and callable(base_module.unmerge_adapter):
            try:
                base_module.unmerge_adapter()
            except Exception:
                pass

        # Normalize keys for vLLM: drop PEFT prefixes like "base_model.model." and "modules_to_save.default."
        normalized_weights = {}
        for key, val in actor_weights.items():
            ...
            if new_key.startswith("modules_to_save.default."):
                new_key = new_key[len("modules_to_save.default.") :]
            # Replace base_layer paths from LoraLinear with plain module path
            new_key = new_key.replace(".base_layer.", ".")
            normalized_weights[new_key] = val
        actor_weights = normalized_weights
