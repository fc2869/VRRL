...
# transformers >= 4.57 restructured the VL configs so that a checkpoint saved by an older
# transformers can report its text-backbone model_type (e.g. "qwen2_5_vl_text") as the
# top-level config.model_type. Normalize those to the VL parent so the dispatch below works.
_VL_TEXT_ALIASES = {
...
def apply_ulysses_patch(model_type: str) -> None:
    ...
    model_type = _VL_TEXT_ALIASES.get(model_type, model_type)
