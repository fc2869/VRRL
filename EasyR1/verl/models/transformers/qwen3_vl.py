...
def get_rope_index(
    ...
    **kwargs,
    ...
    # Qwen3-VL expects [text_position_ids; t; h; w] -> (4, seq_len)
    if attention_mask is not None:
        ...
        text_position_ids = torch.arange(
            position_ids.size(-1), device=position_ids.device, dtype=position_ids.dtype
        )
...
def _get_input_embeds(
    ...
    def _tolerant_scatter_inputs(
        ...
        """
        Replace embeddings at mask positions with features, tolerating mismatched counts
        by using the first min(mask_count, features_count).
        """
    ...
    def _trim_mask(mask_bool: torch.Tensor, keep_count: int) -> torch.Tensor:
        flat_mask = mask_bool.view(-1)
        pos = torch.nonzero(flat_mask, as_tuple=False).squeeze(-1)
        if keep_count >= pos.numel():
            return mask_bool
        trimmed = torch.zeros_like(flat_mask, dtype=torch.bool)
        trimmed[pos[:keep_count]] = True
        return trimmed.view_as(mask_bool)

    def _trim_deepstack(deepstack_embeds, keep_count: int):
        if deepstack_embeds is None:
            return None
        return [emb[:keep_count] for emb in deepstack_embeds]
    ...
    if pixel_values is not None:
        ...
        if n_image_tokens != n_image_features:
            ...
            if used_k < n_image_tokens:
                mask = _trim_mask(mask, used_k)
            deepstack_image_embeds = _trim_deepstack(deepstack_image_embeds, used_k)
        ...
        if n_image_tokens == n_image_features:
            ...
            logger.warning(
                "[Qwen3-VL] Image features and image tokens mismatch: tokens=%d, features=%d. "
    ...
    if pixel_values_videos is not None:
        ...
        if n_video_tokens != n_video_features:
            ...
            if used_k < n_video_tokens:
                mask = _trim_mask(mask, used_k)
            deepstack_video_embeds = _trim_deepstack(deepstack_video_embeds, used_k)
        mask_unsqueezed = mask.unsqueeze(-1)
        mask_expanded = mask_unsqueezed.expand_as(inputs_embeds)
        ...
        if n_video_tokens == n_video_features:
            ...
            logger.warning(
                "[Qwen3-VL] Video features and video tokens mismatch: tokens=%d, features=%d. "
                "Will fill %d positions and leave the rest unchanged.",
                n_video_tokens,
                n_video_features,
                used_k,
    ...
    **kwargs,
    ...
    if isinstance(position_ids, torch.Tensor):
        ...
        if (
            ...
            and position_ids.size(0) == attention_mask.size(0)
            and position_ids.size(1) in (3, 4)
        ):
            # Accept batch-first (bsz, channels, seqlen) and convert to (channels, bsz, seqlen).
            position_ids = position_ids.transpose(0, 1)
            kwargs["position_ids"] = position_ids
        if position_ids.size(0) == 3:
            # Expand legacy (t, h, w) mrope ids into [text; t; h; w].
            if attention_mask is not None:
                text_position_ids = attention_mask.long().cumsum(-1) - 1
                text_position_ids.masked_fill_(attention_mask == 0, 1)
                text_position_ids = text_position_ids.to(device=position_ids.device, dtype=position_ids.dtype)
                ...
                text_position_ids = torch.arange(
                    position_ids.size(-1), device=position_ids.device, dtype=position_ids.dtype
                ).view(1, 1, -1).expand(1, position_ids.size(1), -1)
            if text_position_ids.ndim == 2:
                text_position_ids = text_position_ids.unsqueeze(0)
            position_ids = torch.cat([text_position_ids, position_ids], dim=0)
            kwargs["position_ids"] = position_ids
        elif position_ids.size(0) != 4:
