...
class MultiTurnRLHFDataset(Dataset):
    ...
    def _filter_overlong_prompts(self, example: dict[str, Any]) -> bool:
        ...
        Wraps _filter_overlong_prompts_inner with a catch-all so that corrupted
        or truncated images / bad metadata in one example just drop that example
        from the dataset instead of crashing the whole filter pass (which is
        usually running with num_proc > 1 and would lose progress on a crash).
        """
        try:
            return self._filter_overlong_prompts_inner(example)
        except (OSError, Exception) as e:
            print(f"Warning: skipping example due to error: {e}")
            return False

    def _filter_overlong_prompts_inner(self, example: dict[str, Any]) -> bool:
        ...
        if self.image_key in example:
            ...
            try:
                ...
                return False  # skip samples with corrupted/truncated images
